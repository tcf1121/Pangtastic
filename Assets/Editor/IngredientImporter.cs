...
public class IngredientImporter : EditorWindow
{
    private static string csvPath = "Assets/CSV/ingredients.csv"; //csv 저장위치
    private static int startRow = 3; // 데이터 시작 행
    private static int columnCount = 3;

    [MenuItem("PangTastic/Import Ingredients CSV")] // 메뉴 경로
    public static void StartImportIngredients()
    {
        ImportIngredientCSV();
    }

    private static void ImportIngredientCSV()
    {
        ...
        string saveDir = "Assets/ScriptableObject/Ingredients"; // SO 저장 경로
