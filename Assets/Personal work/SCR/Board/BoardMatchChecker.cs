...
namespace SCR_B
{
    ...
    public class BoardMatchChecker : MonoBehaviour
    {
        ...
        private void DamageMatch()
        {
            ...
            {
                _boardData.BlockArray[pos.y, pos.x]?.TakeDamage(BoardManager.GetBoard());
                _boardData.OverlayArray[pos.y, pos.x]?.TakeDamage(BoardManager.GetBoard());
            }
            foreach (var pos in _splashPos)
            {
                _boardData.BlockArray[pos.y, pos.x]?.SplashDamage(BoardManager.GetBoard());
            }
            _matchPos.Clear();
            _splashPos.Clear();
        }
